 ...
 * Uses the extracted progress worker file via Vite worker URL pattern
 */
export function createProgressWorker(): WorkerChannel {
  // Use Vite worker URL pattern to import the separate worker file
  const workerUrl = new URL('./progress.worker.ts', import.meta.url)
